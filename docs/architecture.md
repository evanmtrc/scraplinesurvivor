...
For Phase 2, extract progression and choice generation into separate systems and data tables. For Phase 3, replace the pistol-specific fire method with a weapon interface and per-weapon cooldown state. Keep one scene as the coordinator until complexity warrants scene splits. Use deterministic seeded random input for reproducible balance tests once upgrade randomness arrives.
...
3. Phase 2: XP thresholds and pause → three distinct choices → upgrades and timer pacing.
